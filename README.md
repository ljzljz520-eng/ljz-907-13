...
- **公益放映排期**: 后台为影片设置放映地点、日期、场次与联系人；影片卡片自动展示近期可看的场次，过期场次自动归入历史；删除影片时如存在关联排期将被阻止并提示先处理
...
- **排期管理**：在影片详情弹窗中点击「管理排期」，可为影片新增、编辑、删除放映场次（一个影片可有多场放映），字段包括放映地点、放映日期、场次时间、联系人、联系电话（选填）
- **前台展示**：影片卡片自动显示最近 2 场近期场次，详情弹窗展示全部近期场次及联系人信息
- **历史归档**：场次过期后（按日期+时间自动判定）自动从卡片和近期列表中消失，归入「历史场次」折叠区
- **删除保护**：删除影片时若仍存在关联排期，接口返回 `409` 并提示先处理排期，前端会给出「去处理排期」入口；数据库外键同步约束（RESTRICT）兜底

### 相关接口

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/movies` | 影片列表（每部影片附带 `upcoming_screenings` 近期场次） |
| GET | `/api/movies/{id}` | 影片详情（附带全部 `screenings`，含 `is_upcoming` 标记） |
| DELETE | `/api/movies/{id}` | 删除影片（存在关联排期时返回 409） |
| GET | `/api/screenings?movie_id=&scope=` | 排期列表，`scope` 支持 `upcoming` / `past` / `all` |
| POST | `/api/screenings` | 新增排期 |
| PUT | `/api/screenings/{id}` | 更新排期 |
| DELETE | `/api/screenings/{id}` | 删除排期 |

排期字段：`movie_id`、`location`（放映地点）、`screening_date`（YYYY-MM-DD）、`start_time`（HH:MM）、`contact_name`（联系人）、`contact_phone`（联系电话，选填）。
